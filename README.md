...
当前版本为 1.0.0，提供 Windows x64 安装包。个人使用免费。仓库使用 MIT 许可证，可自由使用、修改和分发。

## 下载与安装

正式安装包发布在 GitHub 的 Releases 页面，文件名为 ModbusTestTool-Setup-x.x.x.exe。
...
- 数据显示：地址、十进制值、十六进制、二进制、32位 IEEE 754 浮点数、ASCII 字符。
- 自动轮询：启动后按设定间隔持续读取并实时刷新数据表。
- 自动扫描可用串口，无需手动输入端口号。
- 操作日志按时间记录每次连接、读写请求，按类型颜色区分（成功/错误/信息）。
- 浅色和深色主题跟随系统自动切换。
